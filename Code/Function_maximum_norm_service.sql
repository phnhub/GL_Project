CREATE FUNCTION maximum_norm_service (
)
RETURNS float AS
BEGIN
	...
	SET	@max = 
		(
		SELECT MAX(TB.Prediction_de_rentabilite)
		...
		(SELECT top 100
			...
			SUM(CAST(nb_entreprises_secteur_service as float))	 / CAST(Somme_total as float) * @coef1  as Prediction_de_rentabilite
...
--DROP FUNCTION maximum_norm_service;
